...
class NgrokClient {
  ...
  async request(method, path, options = {}) {
    try {
      ...
      let clientError;
      if (!error.response) {
        clientError = new NgrokClientError(error.message || 'Ngrok connection error', null, {
          msg: error.message || 'Ngrok connection error',
          error_code: 0,
        });
        throw clientError;
      }
      try {
        ...
        clientError = new NgrokClientError(response.msg, error.response, response);
      } catch (e) {
        clientError = new NgrokClientError(error.response.body, error.response, error.response.body);
      }
      throw clientError;
    }
  }
  ...
  async booleanRequest(method, path, options = {}) {
    try {
      ...
      if (!error.response) {
        throw new NgrokClientError(error.message || 'Ngrok connection error', null, {
          msg: error.message || 'Ngrok connection error',
          error_code: 0,
        });
      }
