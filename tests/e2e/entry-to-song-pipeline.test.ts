...
describe('Entry to Song Pipeline', () => {
  ...
  describe('Step 1: Entry Submission with Safety Screening', () => {
    it(
      'should create a entry and pass through safety screening',
      async () => {
        if (!testUser) {
        ...
        const entryContent =
          'I have been feeling stressed about my work lately and need to find better ways to manage my time and energy.';
        const headers = TestUserHelper.getAuthHeaders(testUser);
        ...
        if (response.data.safetyScreening) {
          expect(['none', 'low', 'medium', 'high', 'critical']).toContain(response.data.safetyScreening.level);
    ...
    it(
      'should reject empty entry content with validation error',
      async () => {
        if (!testUser) {
  ...
  describe('Step 2: Quota and Eligibility Check', () => {
    it(
      'should check user quota before song generation',
      async () => {
        if (!testUser) {
          throw new Error('Test user not available');
        }
  ...
  describe('Step 3: Song Generation from entry', () => {
    it(
      'should generate song from entry with psychological framework',
      async () => {
        if (!testUser) {
          throw new Error('Test user not available');
        }
    ...
    it(
      'should reject song generation with invalid entryId',
      async () => {
        if (!testUser) {
          throw new Error('Test user not available');
        }
  ...
  describe('Step 4: Verify Generated Track in Library', () => {
    it(
      'should retrieve the generated track with lyrics and audio',
      async () => {
    ...
    it(
      'should list track in user music library',
      async () => {
        if (!testUser) throw new Error('Test user not available');
        if (!generatedTrackId) throw new Error('No track generated in previous step');
  ...
  describe('Step 5: Usage Increment Verification', () => {
    it(
      'should have incremented user usage after generation',
      async () => {
        if (!testUser) throw new Error('Test user not available');
        if (!generatedTrackId) throw new Error('No track generated in previous step');
  ...
  describe('Edge Cases', () => {
    it(
      'should enforce idempotency for duplicate generation requests',
      async () => {
        if (!testUser) throw new Error('Test user not available');
