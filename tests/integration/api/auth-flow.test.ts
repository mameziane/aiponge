...
describe('Authentication Flow', () => {
  ...
  describe('JWT Token Validation', () => {
    it('should accept valid JWT on protected endpoint', async () => {
      if (!registeredUser) {
        throw new Error('No registered user available');
      }
  ...
  describe('Token Refresh - Extended', () => {
    ...
    it('should handle concurrent refresh requests safely', async () => {
      ...
      const results = await Promise.allSettled([
        TestUtils.makeRequest(`${SERVICE_URLS.API_GATEWAY}/api/auth/refresh`, { method: 'POST', body: refreshPayload }, TEST_TIMEOUT),
        TestUtils.makeRequest(`${SERVICE_URLS.API_GATEWAY}/api/auth/refresh`, { method: 'POST', body: refreshPayload }, TEST_TIMEOUT),
        TestUtils.makeRequest(`${SERVICE_URLS.API_GATEWAY}/api/auth/refresh`, { method: 'POST', body: refreshPayload }, TEST_TIMEOUT),
      ]);
      ...
      console.log(`✅ Concurrent refresh: ${successes.length} succeeded, ${fulfilled.length - successes.length} rejected`);
    }, TEST_TIMEOUT);
  ...
  describe('Guest User Flow', () => {
    it('should create guest user for anonymous access', async () => {
      const response = await TestUtils.makeRequest(
    ...
    it('should allow guest user to access protected endpoints', async () => {
    ...
    it('should convert guest to registered user with data migration', async () => {
      if (!guestUser) throw new Error('No guest user created');
  ...
  describe('Role Verification', () => {
    it('should return user role from auth context', async () => {
      if (!registeredUser) {
        throw new Error('No registered user available');
      }
