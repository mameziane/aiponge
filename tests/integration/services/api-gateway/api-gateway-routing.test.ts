...
describe('API Gateway Routing Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    isAPIGatewayAvailable = await TestUtils.waitForServiceHealth(
  ...
  describe('AI Service Routing', () => {
    ...
    it('should route to AI Providers Service correctly', async () => {
      ...
      try {
        ...
        logger.warn('⚠️ AI Providers Service routing failed:', { error: error instanceof Error ? error.message : error });
      }
    });
    ...
    it('should route to AI Analytics Service correctly', async () => {
      ...
      try {
        ...
        logger.warn('⚠️ AI Analytics Service routing failed:', { error: error instanceof Error ? error.message : error });
      }
    });
  });
  ...
  describe('Legacy Route Validation', () => {
    it('should not route to legacy "ai-service" endpoints', async () => {
      ...
      const legacyRouteChecks = legacyRoutes.map(async (route) => {
        try {
          const response = await TestUtils.makeRequest(
            `${SERVICE_URLS.API_GATEWAY}${route}`,
            {}, 
            5000
          );
    ...
    it('should properly handle unknown routes with 404', async () => {
      ...
      const unknownRoutes = [
        '/api/nonexistent-service/health',
        '/api/random/endpoint',
        '/api/test/unknown'
      ];
  ...
  describe('Load Balancing and Failover', () => {
    ...
    it('should maintain consistent routing behavior', async () => {
      ...
      if (uniqueStatuses.length <= 2) { // Allow for some variance (200, 404, etc.)
  ...
  describe('API Gateway Health', () => {
    ...
    it('should provide service routing information', async () => {
      ...
      try {
        const response = await TestUtils.makeRequest(
          `${SERVICE_URLS.API_GATEWAY}/api/health/services`,
          {}, 
          5000
        );
        ...
        logger.warn('⚠️ Could not retrieve routing information:', { error: error instanceof Error ? error.message : error });
      }
    });
  });
  ...
  describe('CORS and Security Headers', () => {
    it('should include appropriate CORS headers', async () => {
      ...
      try {
        const response = await TestUtils.makeRequest(
          `${SERVICE_URLS.API_GATEWAY}/health`,
          {
            headers: {
