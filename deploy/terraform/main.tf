...
locals {
  ...
  services = {
    ...
    "system-service"       = { port = 3001, priority = 10 }
    "storage-service"      = { port = 3002, priority = 20 }
    "user-service"         = { port = 3003, priority = 30 }
    "ai-config-service"    = { port = 3004, priority = 40 }
    "ai-content-service"   = { port = 3005, priority = 50 }
    "ai-analytics-service" = { port = 3006, priority = 60 }
    "music-service"        = { port = 3007, priority = 70 }
  }
}
...
resource "aws_lb_listener" "http" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "{\"status\":\"healthy\",\"service\":\"${var.project_name}\"}"
      status_code  = "200"
    }
  }
}
...
variable "musicapi_api_key" {
  description = "MusicAPI.ai API key"
  type        = string
  sensitive   = true
}
