/**
 * AI Analytics Service Database Connection Factory
 *
 * Uses the consolidated DatabaseConnectionFactory from platform-core
 * with ai-analytics-service specific configuration.
 */

import { createDatabaseConnectionFactory, type SQLConnection, createLogger } from '@aiponge/platform-core';
import type { NeonHttpDatabase } from 'drizzle-orm/neon-http';

const logger = createLogger('ai-analytics-service:database');

export type DatabaseConnection = NeonHttpDatabase<Record<string, unknown>>;
export { SQLConnection };

const factory = createDatabaseConnectionFactory({
  serviceName: 'ai-analytics-service',
  envVarName: 'AI_ANALYTICS_DATABASE_URL',
  fallbackEnvVar: 'DATABASE_URL',
  replicaEnvVar: 'AI_ANALYTICS_DATABASE_REPLICA_URL',
});
