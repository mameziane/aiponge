...
export function registerCompositeRoutes(router: Router): void {
  router.get('/users/:userId/init', async (req: Request, res: Response) => {
    ...
    try {
      const [profileResult, creditsBalanceResult, creditsPolicyResult, guestConversionPolicyResult, recentEntriesResult] =
      ...
      const recentEntries =
        recentEntriesResult.status === 'fulfilled' ? recentEntriesResult.value : [];
