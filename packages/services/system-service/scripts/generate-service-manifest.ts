...
function generateManifest() {
  console.log('🔧 Generating service manifest from services.config.ts...');
  ...
  // Build the content body WITHOUT timestamps so we can compare deterministically
  const servicesJson = JSON.stringify(manifest, null, 2);

  // Check if existing file has the same services array — skip write if unchanged
  // This prevents the pre-commit hook from creating a dirty diff on every commit
  if (fs.existsSync(outputPath)) {
    const existing = fs.readFileSync(outputPath, 'utf-8');
    // Extract the services JSON from the existing file (between "services: " and ",\n  generated:")
    const existingServicesMatch = existing.match(/services:\s*([\s\S]*?),\n\s*generated:/);
    if (existingServicesMatch && existingServicesMatch[1].trim() === servicesJson.trim()) {
      console.log(`ℹ️  Service manifest unchanged, skipping write: ${outputPath}`);
      console.log(`📦 Total services: ${manifest.length}`);
      return;
    }
  }
...
`;
  ...
  console.log(`✅ Service manifest generated: ${outputPath}`);
  console.log(`📦 Total services: ${manifest.length}`);
