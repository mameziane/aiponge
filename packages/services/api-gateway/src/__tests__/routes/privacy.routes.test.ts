...
describe('Privacy Routes', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const mod = await import('../../presentation/routes/app/privacy.routes');
    app = express();
    app.use(express.json());
    app.use('/api/app/privacy', mod.default);
  });

  describe('GET /export', () => {
    it('should return 401 when no user ID', async () => {
      const res = await request(app).get('/api/app/privacy/export');
  ...
  describe('DELETE /data', () => {
    it('should return 401 when no user ID', async () => {
      const res = await request(app).delete('/api/app/privacy/data');
      expect(res.status).toBe(401);
    });
