...
describe('Library Public Routes', () => {
  let app: Express;

  beforeEach(async () => {
    vi.clearAllMocks();
    const mod = await import('../../presentation/routes/app/library-public.routes');
    app = express();
    app.use(express.json());
    app.use('/api/app/library-public', mod.default);
  });
  ...
  describe('GET /public-albums/:albumId', () => {
    it('should return 200 with album details', async () => {
      mockGatewayFetch.mockResolvedValueOnce(
