...
describe('Reflections Routes', () => {
  let app: Express;

  beforeEach(async () => {
    vi.clearAllMocks();
    const mod = await import('../../presentation/routes/app/reflections.routes');
    app = express();
    app.use(express.json());
    app.use('/api/app/reflections', mod.default);
  });
  ...
  describe('POST /', () => {
    ...
    it('should return 201 when creating reflection for authenticated user', async () => {
      mockGatewayFetch.mockResolvedValueOnce(
        mockResponse({ success: true, data: { id: 'ref-123', content: 'My reflection' } }, 201)
      );
      ...
      expect(mockGatewayFetch).toHaveBeenCalledWith(expect.stringContaining('/api/reflections'), expect.anything());
    });
  });

  describe('GET /', () => {
    it('should return 401 when no user ID', async () => {
      const res = await request(app).get('/api/app/reflections');
      expect(res.status).toBe(401);
    });

    it('should return 200 with reflections for authenticated user', async () => {
      mockGatewayFetch.mockResolvedValueOnce(
        mockResponse({ success: true, data: [{ id: 'ref-1', content: 'Reflection 1' }] })
      );
  ...
  describe('GET /:id', () => {
    it('should return 401 when no user ID', async () => {
      const res = await request(app).get('/api/app/reflections/ref-123');
      expect(res.status).toBe(401);
    });

    it('should return 200 with specific reflection for authenticated user', async () => {
      mockGatewayFetch.mockResolvedValueOnce(
  ...
  describe('POST /generate', () => {
    ...
    it('should return 200 with generated reflection for authenticated user', async () => {
      mockGatewayFetch.mockResolvedValueOnce(
        mockResponse({ success: true, data: { questions: ['How are you feeling?'] } })
      );
  ...
  describe('DELETE /:id', () => {
    it('should return 401 when no user ID', async () => {
      const res = await request(app).delete('/api/app/reflections/ref-123');
      expect(res.status).toBe(401);
    });
