...
describe('Store Routes', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const mod = await import('../../presentation/routes/app/store.routes');
    app = express();
  ...
  describe('GET /config', () => {
    it('should return 200 with payment config', async () => {
      ...
      expect(res.body.data.paymentMethod).toBe('in_app_purchase');
    });
  });

  describe('GET /orders', () => {
    it('should return 401 without user-id', async () => {
      const res = await request(app).get('/api/app/store/orders');
