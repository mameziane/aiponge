...
describe('Reminders Routes', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const mod = await import('../../presentation/routes/app/reminders.routes');
    app = express();
    app.use(express.json());
    app.use('/api/app/reminders', mod.default);
  });

  describe('GET /book', () => {
    it('should return 401 when no user ID', async () => {
      const res = await request(app).get('/api/app/reminders/book');
    ...
    it('should return 200 with reminders for authenticated user', async () => {
      ...
      expect(mockGatewayFetch).toHaveBeenCalledWith(expect.stringContaining('/api/reminders/book'), expect.anything());
    });
  ...
  describe('DELETE /book/:id', () => {
    it('should return 401 when no user ID', async () => {
      const res = await request(app).delete('/api/app/reminders/book/rem-123');
      expect(res.status).toBe(401);
    });
  ...
  describe('POST /push-token', () => {
    ...
    it('should return 200 on successful push token registration', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(mockGatewayFetch).toHaveBeenCalledWith(expect.stringContaining('/api/push-tokens'), expect.anything());
    });
  });
});
